...
using namespace cv;
void PCAFUSION::setSvmDetector(const cv::Ptr<cv::ml::SVM>& _svm)
{
	ccasvm = _svm;
}

void PCAFUSION::loadSvmDetector(const string & xmlfile)
{
	ccasvm = ml::StatModel::load<ml::SVM>(xmlfile);
}

void PCAFUSION::loadccamatrix(const string& ymlcca)
{
	...
	ccaf.release();
	featurelen = wxcca.cols;
}

int PCAFUSION::getFeatureLen() const
{
	if (featurelen == 0)
	{
		cerr << "NO PCA MATRIX" << endl;
		return -1;
	}
	return featurelen;
}

void PCAFUSION::compute(const cv::Mat& img, vector<float>& features) const
{
...
void PCAFUSION::detect(const cv::Mat& img, vector<cv::Point>& foundlocations, vector<double>& weights, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, const vector<cv::Point>& locations /*= vector<cv::Point>()*/) const
{
		...
		//#pragma omp parallel
		{
			...
			for (int j = 0, n = 0; j < numwinC; ++j)
			{
				for (int i = 0; i < numwinR; ++i, ++n)
				{
					...
					vector<float> pldpkfeature, feature;
					int k;
...
void PCAFUSION::detect(const cv::Mat& img, vector<cv::Point>& foundLocations, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, const vector<cv::Point>& locations /*= vector<cv::Point>()*/) const
{
...
public:
	Parallel_Detection_CCA(const PCAFUSION* _cca, const Mat& _img, double _hitThreshold, Size _winStride, const double* _levelScale,
...
void PCAFUSION::detectMultiScale(const cv::Mat& img, vector<cv::Rect>& foundlocations, vector<double>& weights, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, double nlevels /*= 64*/, double scale0 /*= 1.1*/, double finalThreshold /*= 2.0*/, bool usemeanshift /*= false*/) const
{
	if (ccasvm->empty())
	{
		cerr << "svm error" << endl;
...
void PCAFUSION::detectMultiScale(const cv::Mat& img, vector<cv::Rect>& foundlocations, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, double nlevels /*= 64*/, double scale0 /*= 1.1*/, double finalThreshold /*= 2.0*/, bool usemeanshift /*= false*/) const
{
	vector<double> weights;
	detectMultiScale(img, foundlocations, weights, hitThreshold, winStride, nlevels, scale0, finalThreshold, usemeanshift);
}

void PCAFUSION::groupRectangles(vector<cv::Rect>& rectList, vector<double>& weights, int groupThreshold, double eps) const
{
...
void PCAFUSION::fusionfeature(vector<float>& f1, vector<float>& f2, vector<float>& f3) const
{
	int flenx = hdddetector.getFeatureLen();
	int fleny = pldpkdetector.getFeatureLen();

	arma::fmat fax(&f1[0], 1, flenx, false);
	arma::fmat fay(&f2[0], 1, fleny, false);
	//arma::fmat fwxcca((float*)wxcca.data, wxcca.rows, wxcca.cols, false);
	//arma::fmat fwycca((float*)wycca.data, wycca.rows, wycca.cols, false);
	arma::fmat fz = fax*fwxcca + fay*fwycca;

	arma::frowvec f = arma::vectorise(fz, 1);
	f3 = arma::conv_to<vector<float> >::from(f);
	/*Mat fx(1, flenx, CV_32FC1, &f1[0]);
	Mat fy(1, fleny, CV_32FC1, &f2[0]);

	Mat fz = fx*wxcca + fy*wycca;
	f3.resize(featurelen);
	memcpy(&f3[0], (float*)fz.data, sizeof(float)*featurelen);*/
}
