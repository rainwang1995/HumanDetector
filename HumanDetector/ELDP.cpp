...
void ELDP::setSvmDetector(const cv::Ptr<cv::ml::SVM>& _svm)
{
	ldpksvm = _svm;
}

void ELDP::loadSvmDetector(const string& xmlfile)
{
	ldpksvm = ml::StatModel::load<ml::SVM>(xmlfile);
}

void ELDP::compute(const cv::Mat& img, vector<float>& features) const
{
	Mat ltpimg;
...
void ELDP::detect(const cv::Mat& img, vector<cv::Point>& foundlocations, vector<double>& weights, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, const vector<cv::Point>& locations /*= vector<cv::Point>()*/) const
{
...
void ELDP::detect(const cv::Mat& img, vector<cv::Point>& foundLocations, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, const vector<cv::Point>& locations /*= vector<cv::Point>()*/) const
{
	vector<double> weights;
	detect(img, foundLocations, weights, hitThreshold, winStride, locations);
}

class Parallel_Detection_ELDP :public ParallelLoopBody
{
private:
	const ELDP* ldpk;
	Mat img;
...
public:
	Parallel_Detection_ELDP(const ELDP* _ldpk, const Mat& _img, double _hitThreshold, Size _winStride, const double* _levelScale,
...
void ELDP::detectMultiScale(const cv::Mat& img, vector<cv::Rect>& foundlocations, vector<double>& weights, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, double nlevels /*= 64*/, double scale0 /*= 1.1*/, double finalThreshold /*= 2.0*/, bool usemeanshift /*= false*/) const
{
	if (ldpksvm->empty())
	{
		cerr << "svm error" << endl;
	...
	parallel_for_(Range(0, levelScale.size()),
		Parallel_Detection_ELDP(this, img, hitThreshold, winStride, &levelScale[0], &foundlocations, &mtx, &weights, &foundScales));
...
void ELDP::detectMultiScale(const cv::Mat& img, vector<cv::Rect>& foundlocations,
...
void ELDP::setDefaultParams()
{
	winSize = Size(64, 128);
	blockSize = Size(16, 16);
}

void ELDP::cal_params()
{
...
void ELDP::compute_Ltpvalue(const cv::Mat& src, cv::Mat& ltpimg) const
{
...
void ELDP::compute_histblock(const cv::Mat& blockltpimg, float* feature) const
{
...
void ELDP::compute_histwin(const cv::Mat& ltpimg, vector<float>& features) const
{
	features.clear();
...
//no-implemention
void ELDP::compute_histimg(const cv::Mat& ltpimg, vector<float>& features, Size winStride) const
{
	Size stride;
	if (winStride.width > blockSize.width || winStride.height>blockSize.height)
	{
...
void ELDP::getblockhist(const cv::Mat& blockimg, cv::Point pt, float* blockhist, vector<vector<float> >& imagehist, vector<bool> flags, int blockperrow) const
{
...
void ELDP::groupRectangles(vector<cv::Rect>& rectList, vector<double>& weights, int groupThreshold, double eps) const
{
...
void ELDP::normalizeBlockHistogram(float* blockhist) const
{
