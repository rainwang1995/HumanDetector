...
void teleainpaint(const CvArr* _input_img, const CvArr* _inpaint_mask, CvArr* _output_img, double inpaintRange, float alpha)
{

    cv::Ptr<CvMat> mask, mask16, band, f, t, out, depth;
	//CvMat* mask, *mask16, *band, *f, *t, *out, *depth;

	//CvPriorityQueueFloat *Out,*Heap;
   ...
   //cv::Ptr<IplConvKernel> el_cross, el_range;
    ...
    depth = cvCreateMat(input_img->rows,input_img->cols, CV_16UC1);
	...
	//icvTeleaInpaintFMM(depth, f, t, out, range, Heap, alpha);
	//cvCopy(out,output_img);
	...
	//cvReleaseMat(&t);
	//cvReleaseMat(&depth);
	//cvReleaseMat(&input_img);
	//cvReleaseMat(&inpaint_mask);
...
void inpaint(cv::InputArray _src, cv::InputArray _mask, cv::OutputArray _dst, double inpaintRange, float alpha)
{
    ...
    teleainpaint(&c_src, &c_mask, &c_dst, inpaintRange, alpha);
}
