...
class LDPK :public DetectionAlgorithm
{
public:
	LDPK() :featurelenblock(56) { setDefaultParams(); cal_params(); }
	LDPK(cv::Size _winSize, cv::Size _blockSize, int _k = 3) :featurelenblock(56),winSize(_winSize), blockSize(_blockSize), K(_k) 
	{ cal_params(); }
