...
void LTDP2::compute_Ltpvalue(const cv::Mat& src, cv::Mat& ltpimgpos,cv::Mat& ltpimgneg)const
{
...
void LTDP2::compute_histblock(const cv::Mat& ltppos,const cv::Mat& ltpneg, float* feature)const
{
...
void LTDP2::compute_histwin(const cv::Mat& ltppos, const cv::Mat& ltpneg, vector<float>& features)const
{
...
void LTDP2::groupRectangles(vector<cv::Rect>& rectList, vector<double>& weights, int groupThreshold, double eps) const
{
...
void LTDP2::compute_Ltpimg(const cv::Mat& src, cv::Mat& uniformp, cv::Mat& uniformn) const
{
	Mat ltppos, ltpneg;
...
void LTDP2::compute(const cv::Mat & img, vector<float>& features) const
{
	Mat ltppos, ltpneg;
...
void LTDP2::setSvmDetector(const cv::Ptr<cv::ml::SVM>& _svm)
{
	ltdpsvm = _svm;
	svmvec.clear();
...
void LTDP2::loadSvmDetector(const string& xmlfile)
{
...
void LTDP2::detect(const cv::Mat& img, vector<cv::Point>& foundlocations, 
...
void LTDP2::detect(const cv::Mat& img, vector<cv::Point>& foundLocations, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, const vector<cv::Point>& locations /*= vector<cv::Point>()*/) const
{
	vector<double> weights;
	detect(img, foundLocations, weights, hitThreshold, winStride, locations);
}

class Parallel_Detection_LTDP2 :public ParallelLoopBody
{
private:
	const LTDP2* ltdp;
	Mat img;
...
public:
	Parallel_Detection_LTDP2(const LTDP2* _ltdp, const Mat& _img, double _hitThreshold, Size _winStride, const double* _levelScale,
...
void LTDP2::detectMultiScale(const cv::Mat& img, vector<cv::Rect>& foundlocations, vector<double>& weights, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, double nlevels /*= 64*/, double scale0 /*= 1.05*/, double finalThreshold /*= 2.0*/, bool usemeanshift /*= false*/) const
{
	if (ltdpsvm->empty())
	{
		cerr << "svm error" << endl;
	...
	parallel_for_(Range(0, levelScale.size()),
		Parallel_Detection_LTDP2(this, img, hitThreshold, winStride, &levelScale[0], &foundlocations, &mtx, &weights, &foundScales));
...
void LTDP2::detectMultiScale(const cv::Mat& img, vector<cv::Rect>& foundlocations, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, double nlevels /*= 64*/, double scale0 /*= 1.05*/, double finalThreshold /*= 2.0*/, bool usemeanshift /*= false*/) const
{
	vector<double> weights;
