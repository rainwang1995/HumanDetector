...
using namespace cv;
void PELDP::setSvmDetector(const cv::Ptr<cv::ml::SVM>& _svm)
{
	ldpksvm = _svm;
}

void PELDP::setDefaultParams()
{
	winSize = Size(64, 128);
	plevels = 3;
}

void PELDP::cal_params()
{
...
void PELDP::compute_Ltpvalue(const cv::Mat& src, cv::Mat& ltpimg) const
{
	...
	for (int i = 0; i < src.rows; ++i)
	{
		for (int j = 0; j < src.cols; ++j)
		{
			...
			ltpimg.at<uchar>(i, j) = sortindex[0] * 8 + sortindex[1];
		}
	}
...
//计算积分图
void PELDP::compute_integralmap(const cv::Mat& ltpimg, vector<cv::Mat>& integralmaps) const
{
...
int PELDP::compute_histvalue(const cv::Mat& integralmap, cv::Rect pos) const
{
...
void PELDP::compute_hists(const vector<cv::Mat>& integralmaps, cv::Rect pos, float* hist) const
{
...
void PELDP::normalise_hist(float* blockhist) const
{
...
void PELDP::compute_histwin(const vector<cv::Mat>& integralmap, cv::Rect roi, vector<float>& features) const
{
	...
	{
		int blocksperrow = pow(2, l);
		int blockspercol = pow(2, l);
...
void PELDP::compute_integralmaps(const cv::Mat& src, vector<cv::Mat>& integralmaps) const
{
	Mat ltpimg;
...
void PELDP::groupRectangles(vector<cv::Rect>& rectList, vector<double>& weights, int groupThreshold, double eps) const
{
...
void PELDP::loadSvmDetector(const string& xmlfile)
{
	ldpksvm = ml::StatModel::load<ml::SVM>(xmlfile);
}

void PELDP::compute(const cv::Mat& img, vector<float>& features) const
{
...
void PELDP::detect(const cv::Mat& img, vector<cv::Point>& foundlocations, vector<double>& weights, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, const vector<cv::Point>& locations /*= vector<cv::Point>()*/) const
{
...
void PELDP::detect(const cv::Mat& img, vector<cv::Point>& foundLocations, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, const vector<cv::Point>& locations /*= vector<cv::Point>()*/) const
{
	vector<double> weights;
	detect(img, foundLocations, weights, hitThreshold, winStride, locations);
}

class Parallel_Detection_PELDP :public ParallelLoopBody
{
private:
	const PELDP* ldpk;
	Mat img;
...
public:
	Parallel_Detection_PELDP(const PELDP* _ldpk, const Mat& _img, double _hitThreshold, Size _winStride, const double* _levelScale,
...
void PELDP::detectMultiScale(const cv::Mat& img, vector<cv::Rect>& foundlocations, vector<double>& weights, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, double nlevels /*= 64*/, double scale0 /*= 1.1*/, double finalThreshold /*= 2.0*/, bool usemeanshift /*= false*/) const
{
	if (ldpksvm->empty())
	{
		cerr << "svm error" << endl;
	...
	parallel_for_(Range(0, levelScale.size()),
		Parallel_Detection_PELDP(this, img, hitThreshold, winStride, &levelScale[0], &foundlocations, &mtx, &weights, &foundScales));
...
void PELDP::detectMultiScale(const cv::Mat& img, vector<cv::Rect>& foundlocations, double hitThreshold /*= 0*/, cv::Size winStride /*= cv::Size()*/, double nlevels /*= 64*/, double scale0 /*= 1.05*/, double finalThreshold /*= 2.0*/, bool usemeanshift /*= false*/) const
{
	vector<double> weights;
