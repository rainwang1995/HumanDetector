...
//using namespace cv;
using namespace std;
class SLTP2 :public DetectionAlgorithm
{
public:
	SLTP2()
	{
	...
	SLTP2(cv::Size _winSize, cv::Size _cellSize, int _signThreshold = 30)
	...
	virtual ~SLTP2() { maskdx.release(), maskdy.release(); sltpsvm.release(); }
private:
	...
	void compute_histwin_thr(const cv::Mat& imgdx, const cv::Mat& imgdy, vector<float>& hist)const;
